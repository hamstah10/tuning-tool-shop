 ...
 * {nnt3:frontendUser.get()->f:variable.set(name:'feUser')}
	...
	private function getFrontendUser(): \TYPO3\CMS\Frontend\Authentication\FrontendUserAuthentication|null
	{
		...
		$context = GeneralUtility::makeInstance(Context::class);
		$userAspect = $context->getAspect('frontend.user');
		if (!$userAspect) return null;

		/** @var ServerRequest $serverRequest */
		$serverRequest = $GLOBALS['TYPO3_REQUEST'] ?? null;
		if (!$serverRequest) return null;

		/** @var \TYPO3\CMS\Frontend\Authentication\FrontendUserAuthentication $frontendUser */
		return $serverRequest->getAttribute('frontend.user', null);
	}
}
