...
class Endpoint extends \Nng\Nnhelpers\Singleton {
	 ...
	 * ```
	 * \nn\rest::Endpoint()->findForRoute( 'get', '/api/test/to/some/{name}/{uid}/somewhere' );
	 * ```
	 * @return array
	 */
	public function findForRoute( $reqType = '', $path = '' ) {
		...
		// Strip the API prefix from the path (e.g., "/api/locations" → "/locations")
		$apiPrefix = rtrim(\nn\rest::Settings()->getApiUrlPrefix(), '/');
		if ($apiPrefix && strpos($path, $apiPrefix) === 0) {
