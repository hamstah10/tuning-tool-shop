...
use Nng\Nnrestapi\Utilities\Log;
...
class rest {
	...
	/**
	 * ```
	 * \nn\rest::Log( $request )->methodName();
	 * ```
	 * @return Log
	 */
	public static function Log( $request = null ) {
		return Log::makeInstance( $request );
	}
	 ...
	 * \nn\rest::ApiError( 'Damn.', 403, 403123, $error );
	 * \nn\rest::ApiError( $throwableError );
	 *
	 * // example with try/catch
	 * try {
	 * 	doSomethingFatal()
	 * } catch(\Throwable $e) {
	 * 	\nn\rest::ApiError( $e, true );
	 * }
	 * ```
	 * @param string $text     		Error message
	 * @param string $statusCode    http status code
	 * @param string $errorCode     custom code for JSON response
	 * @param boolean|object $log   TRUE if the error should be logged in the TYPO3 log (or pass \Throwable)
	 * @return void
	 */
	public static function ApiError( $message = '', $statusCode = 0, $customErrorCode = '', $log = false ) {
		if (is_a($message, ApiError::class)) {
			$log = $statusCode;
			$statusCode = $message->getCode();
			$customErrorCode = $message->getCustomErrorCode();
			$message = $message->getMessage();
		}
		if ($log) {
			if (is_a( $log, '\Throwable', true )) {
				$message .= $log->getMessage() . ' in ' . $log->getFile() . '::' . $log->getLine();
			}
