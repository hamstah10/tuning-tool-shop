...
class Route
{
	...
	public function parse() {
		...
		// Strip API prefix if user included it (e.g., "api/locations" → "locations")
		// This way both @Api\Route("/locations") and @Api\Route("/api/locations") work
		$apiPrefix = trim(\nn\rest::Settings()->getApiUrlPrefix(), '/');
		if ($apiPrefix && strpos($route, $apiPrefix . '/') === 0) {
			$route = substr($route, strlen($apiPrefix) + 1);
		} elseif ($apiPrefix && $route === $apiPrefix) {
			$route = '';
		}
		...
		// Build regex that matches the exact route (anchored at start and end)
