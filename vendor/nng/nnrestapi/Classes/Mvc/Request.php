...
class Request {
	 ...
	 * 
	 * Uses TYPO3's NormalizedParams which handles reverse proxy scenarios
	 * based on the system configuration ($GLOBALS['TYPO3_CONF_VARS']['SYS']['reverseProxyIP'], etc.)
	 * 
	 * @return string
	 */
	public function getRemoteAddr() {
		$normalizedParams = $this->mvcRequest?->getAttribute('normalizedParams');
		if ($normalizedParams) {
			return $normalizedParams->getRemoteAddress();
		}
		return $this->getServerParams()['REMOTE_ADDR'] ?? ($_SERVER['REMOTE_ADDR'] ?? '');
	}
	...
	/**
	 * @param   \TYPO3\CMS\Core\Http\ServerRequest  $mvcRequest  
	 * @return  self
	 */
	...
	/**
	 * Returns `true` if the current request is a request
	 * to an api endpoint, e.g. `/api/...` or `/en/api/...`
	 * 
	 * Does not evaluate if the endpoint is valid or not.
	 * 
	 * ```
	 * if (\Nng\Nnrestapi\Mvc\Request::isApiRequest()) {
	 * 	// do something
	 * }
	 * ```
	 * @return boolean
	 */
	public static function isApiRequest( $request = null ) 
	{
		$request = $request ?: \nn\rest::Settings()->getRequest();
		$site = $request ? $request->getAttribute('site') : false;

		if (!$site) {
			return false;
		}

		// get api prefix (e.g. '/api')
		$basePath = $site->getConfiguration()['nnrestapi']['routing']['basePath'] ?? false;
		...
		// get current lang-prefix in URI, e.g. '/en'
		...
		$languageBaseUri = rtrim($site->getConfiguration()['languages'][$languageId]['base'] ?? '', '/');
		
		return (strpos($path, $basePath.'/') === 0 || strpos($path, $languageBaseUri . $basePath.'/') === 0);
	}
}